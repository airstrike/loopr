...
Public Sub LooprFor(Optional ByVal LooprName As String = "!") '! is a safe "null" equivalent since named ranges can't use it
    ...
    If LooprName = "!" Then LooprName = ""
    
    Dim Nm As Name, Nm2 As Name, Inputs As Collection, CurrentSheet As Worksheet
    Set CurrentSheet = ActiveSheet
    For Each Nm In ActiveWorkbook.Names
        Set Nm2 = Nm
        If InStr(Nm.Name, "_") = 0 Then 'Left(Nm.Name, Len(LOOPR_NAME)) = LOOPR_NAME Then
            If LooprName <> "!" And _
                Left(Nm.Name, Len(LOOPR_NAME & LooprName)) <> (LOOPR_NAME & LooprName) And _
                Nm.Name <> LooprName _
                Then GoTo NextName
            On Error Resume Next
...
Public Sub CaseSwitch(ByVal Direction As Long)
    Dim SU As Long
    SU = Application.ScreenUpdating
    Application.ScreenUpdating = UPDATE_SCREEN
    'On Error Resume Next
    ...
    If CurrentCase = 1 And Direction < 0 Then
        [Case] = Cases(UBound(Cases))
    Else
        For xCase = LBound(Cases) To UBound(Cases)
            If Cases(xCase) = CurrentCase And Direction > 0 Then [Case] = Cases(xCase Mod UBound(Cases) + Direction)
            If Cases(xCase) = CurrentCase And Direction < 0 Then [Case] = Cases(xCase + Direction)
        Next
    End If
    Call Brew
    Application.StatusBar = "Current Case: " & [Case] & " (" & [CaseName] & ") "
    Call SetTimer("CaseSwitch_exit", "00:00:03")
    Application.ScreenUpdating = SU
End Sub

Private Sub CaseSwitch_exit()
    Application.StatusBar = False
End Sub
